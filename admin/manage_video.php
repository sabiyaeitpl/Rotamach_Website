    ...
    if(isset($_POST['submit'])){
        ...
        if($msg==''){
            ...
            if($_FILES['image']['name']!=''){
                    ...
                    //  if (!move_uploaded_file($_FILES['image']['tmp_name'], VIDEO_IMAGE_SERVER_PATH . $image)) {
                    //     throw new Exception("Failed to move the uploaded file.");
                    // }else{
                    //     move_uploaded_file($_FILES['image']['tmp_name'],VIDEO_IMAGE_SERVER_PATH.$image);
                    // }
                    mysqli_query($con,"update $table set heading='$heading',content='$content',video='$video',image='$image' where id='$id'");
                }
                else{
                    mysqli_query($con,"update $table set heading='$heading',content='$content',video='$video' where id='$id'");
                }
                ...
                mysqli_query($con,"insert into $table(heading,content,video,image,status) values('$heading','$content','$video','$image','1')");
            }
        //	header('location:banner.php');
                ...
                <div class="card mb-4">
                        <div class="card-body">
                            ...
                            <form action="" method="post" enctype="multipart/form-data" class="needs-validation tooltip-label-right" >
                                ...
                                <div class="form-group row">
                                    <label class="col-sm-2 col-form-label">Video(Youtube Link)</label>
                                    <div class="col-sm-10">
                                        <textarea name="video" id="" cols="30" rows="6" class="form-control"><?php echo $video ?></textarea>
                                    </div>
                                </div>
                                <div class="form-group row">
                              ...
                              if ($image!='') {
                              	echo "<a target='_blank' href='".VIDEO_IMAGE_SITE_PATH.$image."'><img width='100px' height='100px' src='".VIDEO_IMAGE_SITE_PATH.$image."'/></a>";
                              }
									 ?>
