    ...
    if(isset($_POST['submit'])){
        ...
        if($msg==''){
            ...
            if($_FILES['image']['name']!=''){
                    ...
                    //  if (!move_uploaded_file($_FILES['image']['tmp_name'], TEAM_IMAGE_SERVER_PATH . $image)) {
                    //     throw new Exception("Failed to move the uploaded file.");
                    // }else{
                    //     move_uploaded_file($_FILES['image']['tmp_name'],TEAM_IMAGE_SERVER_PATH.$image);
                    // }
                    mysqli_query($con,"update $table set name='$name',designation='$designation',image='$image' where id='$id'");
                }
                else{
                    mysqli_query($con,"update $table set name='$name',designation='$designation' where id='$id'");
                }
                ...
                mysqli_query($con,"insert into $table(name,designation,image,status) values('$name','$designation','$image','1')");
            }
        //	header('location:banner.php');
                        ...
                        <div class="card-body">
                            ...
                            <form action="" method="post" enctype="multipart/form-data" class="needs-validation tooltip-label-right" >
                                ...
                                <div class="form-group row">
                                    ...
                                    <div class="col-sm-10">
                                        <textarea class="form-control" rows="4"  name="designation" required><?php echo $designation ?></textarea>
                                    </div>
                                </div>
                              ...
                              if ($image!='') {
                              	echo "<a target='_blank' href='".TEAM_IMAGE_SITE_PATH.$image."'><img width='100px' height='100px' src='".TEAM_IMAGE_SITE_PATH.$image."'/></a>";
                              }
									 ?>
